...
export default function AdminMessagesPage() {
    ...
    return (
        <div>
            <h1 className="text-2xl font-bold text-gray-900 mb-6">{t("guestMessages")}</h1>
            <div className="bg-white rounded-xl border border-gray-200 divide-y divide-gray-100">
                {conversations.length === 0 && <p className="p-6 text-gray-400 text-sm text-center">{t("noConversations")}</p>}
                {conversations.map(c => (
                    <button key={c.id} onClick={() => setSelectedConv(c.id)} className="w-full text-left px-4 py-4 hover:bg-gray-50 transition-colors flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-gray-500 truncate">{c.messages[c.messages.length - 1]?.text || t("noConversations")}</p>
                        </div>
                    </button>
                ))}
            </div>

            <Modal isOpen={!!selectedConv} onClose={() => setSelectedConv(null)} title={t("chatWith", { name: conv?.guestName || "" })} size="lg">
                {conv && (
                    <div className="flex flex-col h-[500px]">
                        ...
                        <div className="p-4 border-t flex gap-2">
                            <input type="text" placeholder={t("reply")} value={input} onChange={(e) => setInput(e.target.value)}
