...
export default function GuestInfoPage() {
    ...
    return (
        <div className="px-4 py-6 max-w-2xl mx-auto">
            <h1 className="text-xl font-bold text-[var(--color-text-main)] mb-4">Hotel Information</h1>
            ...
            {activeTab === "dining" && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                    <h2 className="text-base font-bold text-[var(--color-text-main)]">Restaurants & Bars</h2>
                    ...
                    <h2 className="text-base font-bold text-[var(--color-text-main)] mt-6">A&apos;la Carte Restaurants</h2>
                    {MOCK_ALACARTE.map(a => (
            ...
            {activeTab === "activities" && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                    <h2 className="text-base font-bold text-[var(--color-text-main)]">Daily Activities & Animations</h2>
            ...
            {activeTab === "kids" && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                    <h2 className="text-base font-bold text-[var(--color-text-main)]">Kids Club</h2>
                    <div className="glass-panel-heavy rounded-2xl p-5">
                        <div className="flex items-center gap-3 mb-3">
                            <span className="text-3xl">🧒</span>
                            <div>
                                <h3 className="font-bold text-[var(--color-text-main)]">Dolphin Kids Club</h3>
                                <p className="text-xs text-[var(--color-text-muted)]">{MOCK_KIDS_CLUB.ageRange} • {MOCK_KIDS_CLUB.openTime}–{MOCK_KIDS_CLUB.closeTime}</p>
                            </div>
                        </div>
                        <p className="text-sm text-[var(--color-text-muted)] mb-3">{MOCK_KIDS_CLUB.description}</p>
                        <div className="glass-panel rounded-xl p-3 mb-3">
                            <p className="text-xs font-bold text-amber-600 mb-1">📋 Drop-off Policy</p>
                            <p className="text-xs text-[var(--color-text-muted)]">{MOCK_KIDS_CLUB.dropOffPolicy}</p>
                        </div>
                    </div>

                    <h3 className="text-sm font-bold text-[var(--color-text-muted)]">Daily Schedule</h3>
            ...
            {activeTab === "cars" && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                    <h2 className="text-base font-bold text-[var(--color-text-main)]">Car Rental</h2>
                    <p className="text-sm text-[var(--color-text-muted)]">Available at the reception desk</p>
                    {MOCK_CAR_RENTAL.map(c => (
                        <div key={c.id} className="glass-panel rounded-2xl p-4 flex items-center justify-between">
                            ...
                            {c.available ? (
                                <Button variant="glass" size="sm" className="text-[var(--color-text-main)] border-gray-200" onClick={() => showToast(`${c.model} reserved!`)}>Reserve</Button>
                            ) : (
